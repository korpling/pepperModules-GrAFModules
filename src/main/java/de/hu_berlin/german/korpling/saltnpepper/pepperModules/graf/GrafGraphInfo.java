...
/**
 * This class contains some methods for playing around with GrAF's IGraph
 * objects. The MASC corpus does not use many of GrAF's features, therefore they
 * are largely unused / unmaintained.
 */
public class GrafGraphInfo {

	/**
	 * prints the ID and label of an IAnnotation, as well the name and type of
	 * the corresponding IAnnotationSpace. prints the name and value of features
	 * belonging to the annotation (via calling printFeatureInfo).
	 */
	public static void printAnnotationInfo(IAnnotation anno) {
		...
		// String graphElementId = anno.getElement().getId(); // get the ID of
		// the node/edge associated with this annotation
	...
	/**
	 * prints the ID, source node ID and destination node ID of an IEdge. print
	 * annotations (via calling printAnnotationInfo), if present. (edges don't
	 * seem to have annotations in MASC 3.0.0)
	 */
	...
	/**
	 * Prints the name and value of a feature. In MASC 3.0.0, features are
	 * attaced to the annotations of nodes.
	 */
	...
	/**
	 * prints various information about an IGraph object (GrAF/MASC), i.e.
	 * printEdgeInfo, printNodeInfo, printRegionInfo, printRootInfo
	 */
	...
	/**
	 * prints information about an INode: node ID, node degree, IDs of in edges
	 * and out edges. prints node annotations via calling printAnnotationInfo
	 * 
	...
	/**
	 * returns a <String, int> map that counts the nodes for each annotation
	 * space
	 */
	...
	/**
	 * prints the ID and primary text of all INodes present in a given
	 * annotation space.
	 */
	...
	/**
	 * prints all annotation spaces that work directly on the primary text and
	 * lists how many IRegions they annotate.
	 */
	...
	/**
	 * returns a <String, int> map that contains the names of annotation spaces
	 * as keys and the number of regions they annotate as values
	 */
	...
	/**
	 * prints the ID and primary text of all IRegions present in a given
	 * annotation space.
	 */
	...
	/**
	 * prints the root id of a graph, if there is one. MASC 3.0.0 documents
	 * don't even seem to have a root!
	 */
	...
	public static void printStandoffHeaderInfo(IGraph graph) {
		...
		if (annoSpaces.isEmpty() == true) {
			...
			printAnnotationSpacesInfo(graph);
	...
	/**
	 * Prints all the syntax tree roots present in an IGraph. The method should
	 * be used on an IGraph that only contains a syntax annotation level (e.g.
	 * "f.ptb").
	 * 
	 * Example output: f.ptb has these sentence roots: ptb-n00313 S ptb-n00030 S
	 * ptb-n00018 FRAG ptb-n00371 S ptb-n00001 NP
	 * 
