...
public class GrafResourceHeader extends ResourceHeader {
	...
	/** constructs a resource header object given the corpus directory path.
	 *  will only work, if the resource header is named 'resource-header.xml'. */ 
	public GrafResourceHeader(String corpusPath) 
	       throws FileNotFoundException {
		super(loadHeaderFile(corpusPath));
	}

	/** constructs a resource header object given the corpus directory path 
	 *  and the resource header file name */ 
	public GrafResourceHeader(String corpusPath, String headerFileName) 
	       throws FileNotFoundException {
	...
	public void printResourceHeaderInfo() throws GrafException {
		...
		for (FileType fileType : fileTypes) {
			System.out.println("AnnotationId: " + fileType.getAnnotationId()); // a.nc, a.seg
			System.out.println("FileId: " + fileType.getFileId()); // e.g. f.seg, f.penn
			System.out.println("Medium: " + fileType.getMedium()); // e.g. xml, txt
			System.out.println("Suffix: " + fileType.getSuffix()); // e.g. seg, penn
		System.out.println();
		}
	}
	

	/** Returns a list of strings representing the types of annotations that 
	 *  are used in a corpus. Note that not all of the documents in a corpus 
	 *  are annotated with them.
	 *  
	 *  NOTE: annotation types (aka file IDs have slightly different names 
	 *  in the resource header (compared to individual document headers). 
	 *  This is a confirmed bug in MASC 3.0.0. Use 
	 *  MascDocumentHeader.getDocumentAnnotationTypes() instead to retrieve the 
	 *  annotation types used in a document. */
	public List<String> getCorpusAnnotationTypes() 
